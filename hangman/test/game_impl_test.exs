defmodule HangmanImplGameTest do
  ...
  test "new game return correct word" do
    game = Game.new_game("egg")

    assert game.letters == ["e", "g", "g"]
    assert game.turns_left == 7
    assert game.game_state == :initializing
    assert length(game.letters) > 0
  end
  ...
  test "state doesn't change if game is :won or :lost" do
    for state <- [:won, :lost] do
      ...
      { new_game, _tally } = Game.make_move(game, "r")
  ...
  defp check_one_move([guess, state, turns, letters, _used], game) do
